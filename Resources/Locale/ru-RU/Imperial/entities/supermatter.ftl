ent-Supermatter = кристалл суперматерии
    .desc = Странно прозрачный и радужно переливающийся кристалл.

ent-SupermatterMonitorConsole = консоль мониторинга суперматерии
    .desc = Консоль, отображающая информацию о ближайшем кристалле суперматерии.

ent-SupermatterShutdownCrystal = кристалл деактивации суперматерии
    .desc = Кристалл, способный деактивировать станционную суперматерию.

ent-CrateSupermatterShutdownCrystal = ящик кристалла деактивации суперматерии
    .desc = Ящик с кристаллом, способным деактивировать станционную суперматерию.

ent-CrateCrateSupermatterCrystal = ящик «Суперматерия У Себя дома»
    .desc = Набор от Supermatt & Co., предоставляющий всё для собственной Суперматерии прямо у Вас дома.

ent-PaperSupermatterCaution = информационный буклет о Суперматерии
    .desc = { ent-Paper.desc }

ent-CrateSupermatterCrystal = ящик кристалла суперматерии
    .desc = Содержит в себе высоко опасный кристалл Суперматерии, способный убить любого, кто его прикоснётся.


supermatter-console-title = Консоль мониторинга суперматерии
supermatter-console-core-status = Статус
supermatter-console-integrity = Прочность
supermatter-console-event = Событие
supermatter-console-thermodynamics = Сенсор температуры
supermatter-console-temperature = Температура
supermatter-console-atmosphere = Сенсор атмосферы
supermatter-console-pressure = Давление

supermatter-console-temp-value = {$value} К
supermatter-console-pressure-value = {$value} кПа

supermatter-event-none-name = —
supermatter-event-none = Суперматерия стабилизирована.

supermatter-event-lightning-name = Молнии
supermatter-event-lightning = Внимание: Зафиксирован энергетический всплеск! Кристалл выпускает молнии!

supermatter-event-radiation-name = Радиация
supermatter-event-radiation = Внимание: Зафиксирован рост радиационного излучения кристалла!

supermatter-event-plasma-name = Плазма
supermatter-event-plasma = Внимание: Зафиксирован выброс горящей плазмы вокруг кристалла!

supermatter-console-flavor-left = Следите за температурой!
supermatter-console-flavor-right = v0.3

supermatter-desc-pristine = Он выглядит целым.
supermatter-desc-scratched = На нём видны небольшие царапины.
supermatter-desc-cracked = На нём видны трещины.
supermatter-desc-badly-cracked = Он покрыт множеством трещин.
supermatter-desc-critical = Он светится и вибрирует!

supermatter-activated = Внимание: Суперматерия была активирована!
supermatter-deactivated = Внимание: Суперматерия была деактивирована!
supermatter-status-active = [color=yellow]Кристалл слегка переливается жёлтыми оттенками.[/color]
supermatter-status-inactive = [color=gray]Цвет кристалла не меняется. Видимо, он неактивен.[/color]

supermatter-warn-10 = Суперматерия на грани разрушения!
supermatter-warn-25 = Внимание! Целостность суперматерии критически низкая! Немедленно стабилизируйте кристалл!
supermatter-warn-50 = Внимание! Суперматерия сильно повреждена.
supermatter-warn-75 = Внимание! Суперматерия получила повреждения.
supermatter-warn-95 = Зарегистрированы незначительные повреждения суперматерии.
supermatter-station-critical = Внимание! Суперматерия станции на грани разрушения! Немедленно стабилизируйте кристалл!

supermatter-station-catastrophe = Внимание! Датчики фиксируют критическое излучение энергии из кристалла суперматерии! Немедленно эвакуируйтесь с инженерного отдела и удалите персонал от кристалла!

book-text-supermatter-caution = [bold]Благодарим за приобретение Набора «Суперматерия У Себя дома» (СУС)![/bold]

        Набор СУС предоставляет покупателю все материалы, необходимые для установки собственной Суперматерии у Вас дома!

        Содержимое:
            - Кристалл Суперматерии.

        ![bolditalic]ВНИМАНИЕ! Кристалл содержится в ЯЩИКЕ ПОВЫШЕННОЙ ОПАСНОСТИ суперматерии.
        Открывайте его ТОЛЬКО тогда, когда перенесёте его на точку, где планируется её содержать![/bolditalic]

        Условия пользования:
        Supermatt & Co. не несёт ответственности за ущерб в случае:
            - Случайной гибели при касании о Суперматерию
            - Разгерметизации помещения с Суперматерией
            - Катастрофы с участием Суперматерии

        - [italic]Прочитав данный документ, Вы автоматически соглашаетесь с условиями пользования Суперматерией Supermatt & Co[/italic].
