ent-MobCorgiIan = Иан
    .desc = Любимое домашнее животное - корги.
    .suffix = { "" }
ent-MobCorgiIanOld = Старый Иан
    .desc = Все еще любимый домашний корги. Любит свои колеса.
    .suffix = { "" }
ent-MobCorgiLisa = Лиза
    .desc = Любимая корги Иана.
    .suffix = { "" }
ent-MobCatRuntime = Рантайм
    .desc = Профессиональный охотник на мышей. Мастер побега.
    .suffix = { "" }
ent-MobCatException = Эксепшен
    .desc = Хорошенько попросите, и, возможно, они дадут вам одну из своих запасных жизней.
    .suffix = { "" }
ent-MobCatFloppa = Шлёпа
    .desc = Он здесь.
    .suffix = { "" }
ent-MobBandito = Бандито
    .desc = Просто маленький глупый парнишка!
    .suffix = { "" }
ent-MobBingus = бингус
    .desc = Бингус, мой любимый...
    .suffix = { "" }
ent-MobMcGriff = МакГрифф
    .desc = Этот пёс может сказать, что здесь чем-то попахивает, и это что-то - ПРЕСТУПЛЕНИЕ!
    .suffix = { "" }
ent-MobPaperwork = Пэйперворк
    .desc = Устроился на новую работу по сортировке книг в библиотеке после того, как его перевели с Космической Станции 13. Он, похоже, очень медленно справляется с этой работой.
    .suffix = { "" }
ent-MobWalter = Уолтер
    .desc = Он обожает химию и угощения. Уолтер.
    .suffix = { "" }
ent-MobPossumMorty = Морти
    .desc = Обитатель станции, Виргинский опоссум. Чувствительный, но стойкий парень.
    .suffix = { "" }
ent-MobRaccoonMorticia = Мортиша
    .desc = Могущественное создание ночи. Ее тени для век всегда на высоте.
    .suffix = { "" }
ent-MobAlexander = Александр
    .desc = Лучший коллега повара.
    .suffix = { "" }
ent-MobFoxRenault = Алиса
    .desc = Верная лиса капитана.
    .suffix = { "" }
ent-MobHamsterHamlet = Гамлет
    .desc = Ворчливый, милый и пушистый хомяк.
    .suffix = { "" }
ent-MobSpiderShiva = Шива
    .desc = Первый защитник станции.
ent-MobKangarooWillow = Вильям
    .desc = Кенгуру-боксер Вильям.
ent-MobMonkeyPunpun = Пун Пун
    .desc = Яркий представитель обезьян с неограниченным доступом к алкоголю.
ent-MobSlimesPet = Смайл
    .desc = Этот шедевр прошел через тысячи экспериментов. Но это самое милое существо в мире. Улыбнись слайм!
ent-MobPossumPoppy = Поппи
    .desc = Это опоссум, маленькое сумчатое животное, питающееся мусором. Он носит соответствующие средства индивидуальной защиты.
ent-MobCorgiIanPup = щенок Иан
    .desc = Любимый щенок корги. Аввв...
ent-MobCrabAtmos = Тропико
    .desc = Благородный и стойкий защитник Атмосии. Viva la Revolution!
ent-MobPossumMortyOld = { ent-MobPossumMorty}
    .desc = { ent-MobPossumMorty.desc }
    .suffix = Старый
ent-MobBreadDog = Бредог
    .desc = Это хлеб или собака? Теперь я понимаю почему в космосе нет тостеров
ent-MobCatCake = Кортик
    .desc = Это тортик. Это кот. Это Кортик.
ent-MobCatKitten = котёнок
    .desc = Маленький и пушистый
ent-MobBaseScurret = слизенёк
    .desc = { ent-MobBaseAncestor.desc }
ent-MobScurret = слизенёк
    .desc = Эти существа, широко известные как Вава, из водно-болотных земель планеты Вава, составляют основную часть "лояльной рабочей силы" Пиццерии Арнольда.
ent-MobBaseEmotionalSupportScurret = слизенёк эмоциональной поддержки
    .desc = { ent-MobBaseScurret.desc }
ent-MobEmotionalSupportScurret = слизенёк эмоциональной поддержки
    .desc = Эти существа, широко известные как Вава, из водно-болотных земель планеты Вава, составляют основную часть "лояльной рабочей силы" Пиццерии Арнольда. Этот здесь временно.