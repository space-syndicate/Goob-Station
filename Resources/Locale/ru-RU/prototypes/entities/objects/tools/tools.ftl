ent-Wirecutter = кусачки
    .desc = Это убивает провод.
    .suffix = { "" }
ent-Screwdriver = отвёртка
    .desc = Крутящий момент промышленного класса в небольшом корпусе отвёртки.
    .suffix = { "" }
ent-Wrench = гаечный ключ
    .desc = Главный инструмент для откручивания и крепления чего бы то ни было куда-нибудь.
    .suffix = { "" }
ent-Crowbar = лом
    .desc = Многофункциональный инструмент для открывания дверей и борьбы с межпространственными захватчиками.
    .suffix = { "" }
ent-CrowbarRed = аварийный лом
    .desc = { ent-Crowbar.desc }
    .suffix = { "" }
ent-Multitool = мультитул
    .desc = Современный инструмент для копирования, хранения и передачи электрических импульсов и сигналов по проводам и машинам.
    .suffix = { "" }
ent-PowerDrill = электродрель
    .desc = Простая дрель с электроприводом.
    .suffix = { "" }
ent-RCD = РСУ
    .desc = Новейшее ручное строительное устройство, которое может быстро размещать и демонтировать стены, полы и шлюзы.
    .suffix = { "" }
ent-RCDEmpty = { ent-RCD }
    .suffix = Пустой
    .desc = { ent-RCD.desc }
ent-RCDExperimental = РСУ
    .suffix = Пустой
    .desc = Эксперементальный РСУ, пассивно восстанавливающий заряды.
ent-RCDAmmo = картридж РСУ
    .desc = Картридж для РСУ.
    .suffix = { "" }
ent-Shovel = лопата
    .desc = Большой инструмент для копания и перемещения грунта.
    .suffix = { "" }
ent-Omnitool = омнитул
    .desc = Лучший друг дрона.
    .suffix = { "" }
ent-NetworkConfigurator = сетевой конфигуратор
    .desc = Инструмент для соединения устройств вместе. Имеет два режима: режим списка для массового связывания устройств и режим связывания для расширенного связывания устройств.
ent-RCDRecharging = РСУ
    .suffix = Борги
    .desc = Новейшее ручное строительное устройство, которое может быстро размещать и демонтировать стены, полы и шлюзы.
ent-RollingPin = скалка
    .desc = Инструмент, используемый для придания формы и раскатывания теста.
ent-ToolboxThief = ящик инструментов вора
    .desc = Здесь лежат припасы вашего любимого вора. Постарайтесь вспомнить, какие именно.
ent-ToyFigurineThief = фигурка вора
    .desc = Скрываясь в тени...
ent-HandHeldMassScanner = ручной сканер массы
    .desc = ручной сканер массы
ent-HandHeldMassScannerBorg = ручной сканер массы
    .desc = ручной сканер массы
    .suffix = Борг
ent-HandHeldMassScannerEmpty = ручной сканер массы
    .desc = ручной сканер массы
    .suffix = Пустой
ent-HijackBeacon = маяк угона
    .desc = Устройство, позволяющее обходить брандмауэр на Торговых Аванпостах Nanotrasen.
    .suffix = { "" }
