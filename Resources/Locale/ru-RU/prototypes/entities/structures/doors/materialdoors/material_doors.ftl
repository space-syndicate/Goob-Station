ent-BaseMaterialDoor = дверь
    .desc = Дверь, куда она ведет?
    .suffix = { "" }
ent-MetalDoor = металлическая дверь
    .desc = { ent-BaseMaterialDoor.desc }
    .suffix = { "" }
ent-WoodDoor = деревянная дверь
    .desc = Дверь, куда она ведет?
    .suffix = { "" }
ent-PaperDoor = бумажная дверь
    .desc = Дверь, куда она ведет?
    .suffix = { "" }
ent-PlasmaDoor = плазменная дверь
    .desc = Дверь, куда она ведет?
    .suffix = { "" }
ent-GoldDoor = золотая дверь
    .desc = Дверь, куда она ведет?
    .suffix = { "" }
ent-SilverDoor = серебряная дверь
    .desc = Дверь, куда она ведет?
    .suffix = { "" }
ent-BananiumDoor = бананиумовая дверь
    .desc = Дверь, она ведет к бананам?
    .suffix = { "" }
ent-WebDoor = дверь из паутины
    .desc = Дверь, ведущая в страну пауков... или в отдельную комнату.
    .suffix = { "" }
ent-EncrustedIronstoneDoor = засохшая дверь из железного песка
    .desc = Каменная дверь, покрытая перламутровыми пятнами неизвестного вещества.
ent-IronstoneDoor = дверь из железного камня
    .desc = Интересная дверь, сделанная из изрезанного рунами камня.
