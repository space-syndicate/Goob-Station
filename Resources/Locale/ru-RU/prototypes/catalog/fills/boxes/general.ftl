ent-BigBox = картонная коробка
    .desc = Картонная коробка для хранения вещей.
    .suffix = { "" }
ent-BoxMousetrap = коробка мышеловок
    .desc = Коробка, наполненая мышеловками. Постарайтесь не поймать себя за руку.
    .suffix = { "" }
ent-BoxLightbulb = коробка лампочек
    .desc = Из-за формы коробки в нее помещаются только лампочки и лампочки-трубки.
    .suffix = { "" }
ent-BoxLighttube = коробка лампочек-трубок
    .desc = Из-за формы коробки в нее помещаются только лампочки и лампочки-трубки.
    .suffix = { "" }
ent-BoxLightMixed = коробка смешанных лампочек
    .desc = Из-за формы коробки в нее помещаются только лампочки и лампочки-трубки.
    .suffix = { "" }
ent-BoxPDA = коробка КПК
    .desc = Коробка запасных микрокомпьютеров КПК.
    .suffix = { "" }
ent-BoxID = коробка ID карт
    .desc = Коробка запасных чистых ID карт.
    .suffix = { "" }
ent-BoxHeadset = коробка гарнитур
    .desc = Коробка запасных пассажирских гарнитур.
    .suffix = { "" }
ent-BoxMesonScanners = коробка инженерных очков
    .desc = Коробка запасных инженерных очков.
    .suffix = { "" }
ent-BoxMRE = И.Р.П.
    .desc = Коробка старых военных пайков. Они, на удивление, не испортились.
    .suffix = { "" }
ent-BoxHugHealing = коробка обнимашек
    .desc = Специальная коробка для чутких людей.
    .suffix = { "" }
ent-BoxInflatable = коробка надувных стен
    .desc = Надувные стены не должны использоваться в качестве плавучих средств.
    .suffix = { "" }
ent-BoxPerformer = коробка в честь дня Хацунэ Мику
    .desc = Счастливого дня Хацунэ Мику!
    .suffix = { "" }
ent-BoxFlare = коробка фальшфейеров
    .desc = Коробка фальшфейеров. Время зажигать.
    .suffix = { "" }
ent-BoxTrashbag = коробка мусорных пакетов
    .desc = Коробка мусорных пакетов. Звуки счастливого уборщика.
    .suffix = { "" }
ent-BoxEncryptionKeyPassenger = коробка ключей шифрования пассажиров
    .desc = Коробка запасных ключей шифрования.
    .suffix = { "" }
ent-BoxEncryptionKeyCargo = коробка ключей шифрования отдела снабжения
    .desc = { ent-BoxEncryptionKeyPassenger.desc }
    .suffix = { "" }
ent-BoxEncryptionKeyEngineering = коробка ключей шифрования инженерного отдела
    .desc = { ent-BoxEncryptionKeyPassenger.desc }
    .suffix = { "" }
ent-BoxEncryptionKeyMedicalScience = коробка ключей шифрования медико-научных
    .desc = { ent-BoxEncryptionKeyPassenger.desc }
    .suffix = { "" }
ent-BoxEncryptionKeyMedical = коробка ключей шифрования медицинского отдела
    .desc = { ent-BoxEncryptionKeyPassenger.desc }
    .suffix = { "" }
ent-BoxEncryptionKeyRobo = коробка ключей шифрования роботехников
    .desc = { ent-BoxEncryptionKeyPassenger.desc }
    .suffix = { "" }
ent-BoxEncryptionKeyScience = коробка ключей шифрования научного отдела
    .desc = { ent-BoxEncryptionKeyPassenger.desc }
    .suffix = { "" }
ent-BoxEncryptionKeySecurity = коробка ключей шифрования службы безопасности
    .desc = { ent-BoxEncryptionKeyPassenger.desc }
    .suffix = { "" }
ent-BoxEncryptionKeyService = коробка ключей шифрования сервисного отдела
    .desc = { ent-BoxEncryptionKeyPassenger.desc }
    .suffix = { "" }
ent-BoxEncryptionKeySyndie = коробка ключей шифрования синдиката
    .desc = Два синдикатных ключа шифрования по цене одного. Миниатюрный для удобства использования.
    .suffix = { "" }
ent-BoxCardboard = картонная коробка
    .desc = Картонная коробка для хранения вещей.
ent-CrateCoffin = гроб
    .desc = Гроб для хранения трупов.
ent-BoxDeathRattleImplants = коробка для имплантатов предсмертного хрипа
    .desc = Шесть имплантатов с предсмертным хрипом и портативные GPS-навигаторы для всего отряда.

ent-EncryptionKeyBinary = бинарный ключ транслятор
    .desc = Ключ шифрования, который преобразует двоичные сигналы, используемые синтетиками.
ent-BoxLeadLined = Свинцовая коробка
    .desc = Этот ящик блокирует вредное радиоактивное излучение
ent-BoxDarts = коробка дротиков
    .desc = Эта коробка полна цветными дротиками
ent-BoxCandle = коробка свечей
    .desc = картонная коробка для хранения вещей
ent-ChemicalSynthesisKit = набор для химического синтеза
    .desc = Стартовый набор для начинающего химика включает в себя токсин и вестин для всех ваших криминальных нужд!
ent-BoxCandleSmall = маленькая коробка свечей
    .desc = картонная коробка для хранения вещей
ent-BoxEvidenceMarkers = коробка маркеров улик
    .desc = Набор пронумерованных желтых маркеров, которые можно использовать для обозначения улик на месте преступления.
ent-KoboldCubeBox = коробка кубиков кобольдов
    .desc = Содержит кобольдов в кубиках. Просто добавь воды!
ent-BoxVial = коробка пробирок
    .desc = Коробка полная пробирок
ent-ThrowingKnivesKit = комплект метательных ножей
    .desc = Набор из 4-х фирменных метательных ножей синдиката, идеально подходящих для вживления в тела ваших жертв.
ent-ToteBase = коробка для схем
    .desc = Большая коробка для хранения печатных плат.
