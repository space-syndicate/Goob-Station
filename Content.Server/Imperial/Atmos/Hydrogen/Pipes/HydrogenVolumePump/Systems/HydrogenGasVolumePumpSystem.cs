...
using Content.Shared.Imperial.Atmos.Piping.Binary.Systems;
...
{
    [UsedImplicitly]
    public sealed class HydrogenGasVolumePumpSystem : SharedHydrogenGasVolumePumpSystem
    {
        ...
        public override void Initialize()
        {
            ...
            SubscribeLocalEvent<HydrogenGasVolumePumpComponent, AtmosDeviceUpdateEvent>(OnVolumePumpUpdated);
            SubscribeLocalEvent<HydrogenGasVolumePumpComponent, AtmosDeviceDisabledEvent>(OnVolumePumpLeaveAtmosphere);

            SubscribeLocalEvent<HydrogenGasVolumePumpComponent, DeviceNetworkPacketEvent>(OnPacketRecv);
        }

        private void OnVolumePumpUpdated(EntityUid uid, HydrogenGasVolumePumpComponent pump, ref AtmosDeviceUpdateEvent args)
        {
            if (!pump.Enabled ||
        ...
        private void OnVolumePumpLeaveAtmosphere(EntityUid uid, HydrogenGasVolumePumpComponent pump, ref AtmosDeviceDisabledEvent args)
        {
            pump.Enabled = false;
            Dirty(uid, pump);
            UpdateAppearance(uid, pump);
            _userInterfaceSystem.CloseUi(uid, HydrogenGasVolumePumpUiKey.Key);
        }

        private void OnPacketRecv(EntityUid uid, HydrogenGasVolumePumpComponent component, DeviceNetworkPacketEvent args)
        {
            ...
            switch (cmd)
            {
                case AtmosDeviceNetworkSystem.SyncData:
                    ...
                    payload.Add(AtmosDeviceNetworkSystem.SyncData, new HydrogenGasVolumePumpData(component.LastMolesTransferred));
