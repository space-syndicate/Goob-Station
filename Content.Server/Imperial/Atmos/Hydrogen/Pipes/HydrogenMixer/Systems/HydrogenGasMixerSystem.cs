...
{
    [UsedImplicitly]
    public sealed class HydrogenGasMixerSystem : EntitySystem
    {
        ...
        public override void Initialize()
        {
            ...
            SubscribeLocalEvent<HydrogenGasMixerComponent, ComponentInit>(OnInit);
            SubscribeLocalEvent<HydrogenGasMixerComponent, AtmosDeviceUpdateEvent>(OnMixerUpdated);
            SubscribeLocalEvent<HydrogenGasMixerComponent, ActivateInWorldEvent>(OnMixerActivate);
            SubscribeLocalEvent<HydrogenGasMixerComponent, GasAnalyzerScanEvent>(OnMixerAnalyzed);
            // Bound UI subscriptions
            SubscribeLocalEvent<HydrogenGasMixerComponent, HydrogenGasMixerChangeOutputPressureMessage>(OnOutputPressureChangeMessage);
            SubscribeLocalEvent<HydrogenGasMixerComponent, HydrogenGasMixerChangeNodePercentageMessage>(OnChangeNodePercentageMessage);
            SubscribeLocalEvent<HydrogenGasMixerComponent, HydrogenGasMixerToggleStatusMessage>(OnToggleStatusMessage);

            SubscribeLocalEvent<HydrogenGasMixerComponent, AtmosDeviceDisabledEvent>(OnMixerLeaveAtmosphere);
        }

        private void OnInit(EntityUid uid, HydrogenGasMixerComponent mixer, ComponentInit args)
        {
            UpdateAppearance(uid, mixer);
        }

        private void OnMixerUpdated(EntityUid uid, HydrogenGasMixerComponent mixer, ref AtmosDeviceUpdateEvent args)
        {
        ...
        private void OnMixerLeaveAtmosphere(EntityUid uid, HydrogenGasMixerComponent mixer, ref AtmosDeviceDisabledEvent args)
        {
            mixer.Enabled = false;

            DirtyUI(uid, mixer);
            UpdateAppearance(uid, mixer);
            _userInterfaceSystem.CloseUi(uid, HydrogenGasMixerUiKey.Key);
        }

        private void OnMixerActivate(EntityUid uid, HydrogenGasMixerComponent mixer, ActivateInWorldEvent args)
        {
            ...
            {
                _userInterfaceSystem.OpenUi(uid, HydrogenGasMixerUiKey.Key, actor.PlayerSession);
                DirtyUI(uid, mixer);
            }
            else
            {
        ...
        private void DirtyUI(EntityUid uid, HydrogenGasMixerComponent? mixer)
        {
            if (!Resolve(uid, ref mixer))
                return;

            _userInterfaceSystem.SetUiState(uid, HydrogenGasMixerUiKey.Key,
                new HydrogenGasMixerBoundUserInterfaceState(Comp<MetaDataComponent>(uid).EntityName, mixer.TargetPressure, mixer.Enabled, mixer.InletOneConcentration));
        }

        private void UpdateAppearance(EntityUid uid, HydrogenGasMixerComponent? mixer = null, AppearanceComponent? appearance = null)
        {
        ...
        private void OnToggleStatusMessage(EntityUid uid, HydrogenGasMixerComponent mixer, HydrogenGasMixerToggleStatusMessage args)
        {
        ...
        private void OnOutputPressureChangeMessage(EntityUid uid, HydrogenGasMixerComponent mixer, HydrogenGasMixerChangeOutputPressureMessage args)
        {
        ...
        private void OnChangeNodePercentageMessage(EntityUid uid, HydrogenGasMixerComponent mixer,
            HydrogenGasMixerChangeNodePercentageMessage args)
        {
        ...
        /// </summary>
        private void OnMixerAnalyzed(EntityUid uid, HydrogenGasMixerComponent component, GasAnalyzerScanEvent args)
        {
