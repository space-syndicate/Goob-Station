...
{
    [UsedImplicitly]
    public sealed class HydrogenGasFilterSystem : EntitySystem
    {
        ...
        public override void Initialize()
        {
            ...
            SubscribeLocalEvent<HydrogenGasFilterComponent, ComponentInit>(OnInit);
            SubscribeLocalEvent<HydrogenGasFilterComponent, AtmosDeviceUpdateEvent>(OnFilterUpdated);
            SubscribeLocalEvent<HydrogenGasFilterComponent, AtmosDeviceDisabledEvent>(OnFilterLeaveAtmosphere);
            SubscribeLocalEvent<HydrogenGasFilterComponent, ActivateInWorldEvent>(OnFilterActivate);
            SubscribeLocalEvent<HydrogenGasFilterComponent, GasAnalyzerScanEvent>(OnFilterAnalyzed);
            // Bound UI subscriptions
            SubscribeLocalEvent<HydrogenGasFilterComponent, HydrogenGasFilterChangeRateMessage>(OnTransferRateChangeMessage);
            SubscribeLocalEvent<HydrogenGasFilterComponent, HydrogenGasFilterSelectGasMessage>(OnSelectGasMessage);
            SubscribeLocalEvent<HydrogenGasFilterComponent, HydrogenGasFilterToggleStatusMessage>(OnToggleStatusMessage);

        }

        private void OnInit(EntityUid uid, HydrogenGasFilterComponent filter, ComponentInit args)
        {
            UpdateAppearance(uid, filter);
        }

        private void OnFilterUpdated(EntityUid uid, HydrogenGasFilterComponent filter, ref AtmosDeviceUpdateEvent args)
        {
            if (!filter.Enabled
                ...
                || outletNode.Air.Pressure >= Atmospherics.HydrogenMaxOutputPressure) // No need to transfer if target is full.
            {
            ...
            if (filter.FilteredGas.HasValue)
            {
                ...
                var target = filterNode.Air.Pressure < Atmospherics.HydrogenMaxOutputPressure ? filterNode : inletNode;
        ...
        private void OnFilterLeaveAtmosphere(EntityUid uid, HydrogenGasFilterComponent filter, ref AtmosDeviceDisabledEvent args)
        {
            ...
            DirtyUI(uid, filter);
            _userInterfaceSystem.CloseUi(uid, HydrogenGasFilterUiKey.Key);
        }

        private void OnFilterActivate(EntityUid uid, HydrogenGasFilterComponent filter, ActivateInWorldEvent args)
        {
        ...
        private void DirtyUI(EntityUid uid, HydrogenGasFilterComponent? filter)
        {
            if (!Resolve(uid, ref filter))
                return;

            _userInterfaceSystem.SetUiState(uid, HydrogenGasFilterUiKey.Key,
                new HydrogenGasFilterBoundUserInterfaceState(MetaData(uid).EntityName, filter.TransferRate, filter.Enabled, filter.FilteredGas));
        }

        private void UpdateAppearance(EntityUid uid, HydrogenGasFilterComponent? filter = null)
        {
        ...
        private void OnToggleStatusMessage(EntityUid uid, HydrogenGasFilterComponent filter, HydrogenGasFilterToggleStatusMessage args)
        {
        ...
        private void OnTransferRateChangeMessage(EntityUid uid, HydrogenGasFilterComponent filter, HydrogenGasFilterChangeRateMessage args)
        {
        ...
        private void OnSelectGasMessage(EntityUid uid, HydrogenGasFilterComponent filter, HydrogenGasFilterSelectGasMessage args)
        {
        ...
        /// </summary>
        private void OnFilterAnalyzed(EntityUid uid, HydrogenGasFilterComponent component, GasAnalyzerScanEvent args)
        {
