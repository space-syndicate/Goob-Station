...
{
    /// <summary>
    /// Initializes a <see cref="HydrogenMixerWindow"/> and updates it when new server messages are received.
    /// </summary>
    [UsedImplicitly]
    public sealed class HydrogenMixerBoundUserInterface : BoundUserInterface
    {
        [ViewVariables]
        private const float MaxPressure = Atmospherics.HydrogenMaxOutputPressure;

        [ViewVariables]
        private HydrogenMixerWindow? _window;

        public HydrogenMixerBoundUserInterface(EntityUid owner, Enum uiKey) : base(owner, uiKey)
        {
        }
        ...
        protected override void Open()
        {
            ...
            _window = this.CreateWindow<HydrogenMixerWindow>();
        ...
        {
            SendMessage(new HydrogenGasMixerToggleStatusMessage(status));
        }
        ...
        private void OnMixerOutputPressurePressed(string value)
        {
            ...
            SendMessage(new HydrogenGasMixerChangeOutputPressureMessage(pressure));
        }
        ...
        private void OnMixerSetPercentagePressed(string value)
        {
            ...
            SendMessage(new HydrogenGasMixerChangeNodePercentageMessage(node));
        }
