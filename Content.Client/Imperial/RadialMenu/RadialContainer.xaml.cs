...
public partial class RadialContainer : Control
{
    ...
    {
        if (uid == EntityUid.Invalid) return;
    ...
    public RadialButton AddButton(string action, EntityUid uid, string? texture = null)
    {
        var button = new RadialButton();
    ...
    public RadialButton AddButton(string action, NetEntity uid, string? texture = null)
    {
        var button = new RadialButton();
    ...
    private void PlayRadialAnimation(Control button, Vector2 pos, string playKey)
    {
        ...
        {
            Length = TimeSpan.FromSeconds(_moveAniTime),
            AnimationTracks =
            {
    ...
    private void PlaySizeAnimation(Control button, Vector2 size, string playKey, string? stopKey)
    {
        var anim = new Animation
        {
            Length = TimeSpan.FromSeconds(_focusAniTime),
            AnimationTracks =
            {
                new AnimationTrackControlProperty
                {
    ...
    protected override void FrameUpdate(FrameEventArgs args)
    {
        ...
        var transformSystem = IoCManager.Resolve<IEntityManager>().System<TransformSystem>();
        ...
        if (!entityManager.TryGetComponent<TransformComponent>(_attachedEntity, out var xform)) return;
        if (xform.MapID != transformSystem.GetMapId(localPlayer.Value)) return;

        // Check distance beetween entities
        if (entityManager.TryGetComponent<TransformComponent>(localPlayer, out var myxform))
        {
            var onePoint = transformSystem.GetWorldPosition(_attachedEntity.Value);
            var twoPoint = transformSystem.GetWorldPosition(localPlayer.Value);
        ...
        var worldPos = transformSystem.GetWorldPosition(_attachedEntity.Value) + offset;
