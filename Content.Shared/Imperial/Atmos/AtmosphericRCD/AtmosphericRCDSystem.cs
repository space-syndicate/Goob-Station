...
public sealed class AtmosphericRCDSystem : EntitySystem
{
        ...
        switch (prototype.Mode)
        {
            case AtmosphericRcdMode.Deconstruct:
                ...
                else
                {
                    ...
                    var protoName = !_turf.IsSpace(deconstructedTile) ? component.DeconstructTileProto : component.DeconstructLatticeProto;
            ...
            case AtmosphericRcdMode.ConstructTile:
                ...
                {
                    delay = component.InstantConstructionDelay;
                    effectPrototype = component.InstantConstructionFx;
                }
    ...
    private bool IsConstructionLocationValid(EntityUid uid, AtmosphericRCDComponent component, EntityUid gridUid, MapGridComponent mapGrid, TileRef tile, Vector2i position, EntityUid user, bool popMsgs = true)
    {
        ...
        if (prototype.ConstructionRules.Contains(AtmosphericRcdConstructionRule.MustBuildOnSubfloor) && !_turf.GetContentTileDefinition(tile).IsSubFloor)
        {
            if (popMsgs)
        ...
        component.IntersectingEntities.Clear();
        _lookup.GetLocalEntitiesIntersecting(gridUid, position, component.IntersectingEntities, -0.05f, LookupFlags.Uncontained);

        foreach (var ent in component.IntersectingEntities)
        {
            if (isWindow && HasComp<SharedCanBuildWindowOnTopComponent>(ent))
                continue;

            if (isCatwalk && _tags.HasTag(ent, component.CatwalkTag))
            {
                if (popMsgs)
